...
@section('per_page_title')
    {{ __('Edit Product | User Dashboard') }}
@endsection
                            ...
                            <div class="col-12">
                                <nav aria-label="breadcrumb">
                                    <ol class="breadcrumb">
                                        ...
                                        <li class="breadcrumb-item"><a href="{{ route('user.products.all') }}">{{ __('Product') }}</a></li>
                                ...
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="card-title mb-0">{{ __('Edit Product') }}</h5>                                    
                                            <a href="{{ route('user.products.all') }}" class="btn btn-outline-dark waves-effect waves-light float-end">{{ __('All Product') }}</a>
                                    
                                    </div>
                                    <div class="card-body">

                                        <form class="ajax-form" action="{{ route('user.products.update', ['id' => encrypt($single_product->id)]) }}" method="post" autocomplete="off" enctype="multipart/form-data">
                                            @csrf
                                            <div class="row">
                                                <!--name start -->
